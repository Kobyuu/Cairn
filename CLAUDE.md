...
- **`setup()` corre DESPUÉS de que se crean las ventanas, así que el webview puede invocar comandos antes de que exista el estado.** Las ventanas declaradas en `tauri.conf.json` se crean durante `build()`, y su webview empieza a cargar en paralelo; `app.manage()` vive en `setup()`, que llega más tarde. Un `invoke` que gane esa carrera falla con ``state not managed for field `state` on command `…` `` — **HTTP 200 igual**, porque en Tauri v2 un comando que devuelve `Err` responde 200 y lo que se rechaza es la promesa de JS. Verificado el 2026-09-04: con `WEBVIEW2_ADDITIONAL_BROWSER_ARGUMENTS=--remote-debugging-port=9222` el webview gana **siempre** (4/4 corridas), y sin eso gana a veces — o sea que es un bug que en `dev` casi no se ve y en la máquina del usuario sí. **Todo pedido de estado inicial desde el frontend tiene que reintentar**, y el reintento no puede tener tope de intentos: un tope solo cambia "colgado para siempre" por "colgado después de N". Vive en `src/useTimer.ts`.
- **Un `console.error` en release es un error que no existe.** El webview de un build de release no tiene devtools, así que un `catch(console.error)` en el arranque es indistinguible del silencio. Para ver la consola de un `.exe` ya compilado: lanzarlo con `WEBVIEW2_ADDITIONAL_BROWSER_ARGUMENTS=--remote-debugging-port=9222` y hablarle por CDP a `http://127.0.0.1:9222/json/list`. Es la única forma de leer lo que la app dice de sí misma en producción.
...
- **La CSP NO se aplica en `pnpm tauri dev`, y el IPC de Windows es `http://ipc.localhost`.** La política sale como cabecera `Content-Security-Policy` del protocolo `tauri://`, que solo sirve los assets de `frontendDist`; con `devUrl` apuntando a Vite, la página la sirve `http://localhost:1420` y no lleva cabecera ninguna. **Probar la CSP exige `pnpm tauri build` y correr el `.exe`.** El `invoke` viaja por `fetch` a `convertFileSrc(cmd, 'ipc')`, que en Windows arma `http://ipc.localhost/<cmd>` (el `ipc:` que muestran los ejemplos es el esquema de macOS/Linux), así que `connect-src` tiene que incluirlo o el `fetch` se bloquea. Ojo: no se rompe del todo — el script inyectado cae al `window.ipc.postMessage`, así que la app **sigue andando** y lo único que avisa es una violación de CSP en la consola del webview. Y el parser de Tauri parte la política por `;` y cada directiva por **un solo espacio**: la cadena va en una línea, sin saltos ni espacios dobles. (Verificado el 2026-09-04 contra el fuente de `tauri` 2.11.5: `manager/mod.rs::get_asset`, `protocol/tauri.rs`, `scripts/core.js`, `scripts/ipc-protocol.js`.)
