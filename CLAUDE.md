...
- **Todo comando y todo plugin necesita permiso declarado en `src-tauri/capabilities/`.** Una *capability* es el archivo JSON que le dice a Tauri qué puede invocar cada ventana. Si falta, la llamada falla **en runtime**, no en compilación — y el error aparece en la consola del webview, no en la terminal de `cargo`. Es la primera cosa a revisar cuando un `invoke` "no hace nada".
